...
//        $status = "notverified";
//
//        $insert_data = "INSERT INTO admin (firstname, lastname, email, password, code, status)
...
//}
   ...
   // if user click login button
    if(isset($_POST['login'])){
        $email = mysqli_real_escape_string($conn, $_POST['email']);
        $password = mysqli_real_escape_string($conn, $_POST['password']);
        $check_email = "SELECT * FROM admin WHERE username = '$email'";
        ...
        if(mysqli_num_rows($res) > 0){
            ...
            $fetch_memID = $fetch['admin_id'];
            $fetch_fname = $fetch['f_name'];
            $fetch_lname = $fetch['l_name'];
